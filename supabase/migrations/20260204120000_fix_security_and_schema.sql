...
-- 3. Check Existing Application RPC (for TCKN status check - bypasses RLS)
CREATE OR REPLACE FUNCTION check_existing_application(
    p_tckn_plain TEXT,
    p_campaign_id UUID,
    p_member_id UUID
)
...
DECLARE
    v_existing_id UUID;
    v_existing_tckn TEXT;
BEGIN
    -- Check by TCKN first (matches RPC logic)
    SELECT id, tckn INTO v_existing_id, v_existing_tckn
    FROM applications 
    WHERE campaign_id = p_campaign_id AND tckn = p_tckn_plain
    LIMIT 1;
    
    IF v_existing_id IS NOT NULL THEN
        RETURN jsonb_build_object('exists', true, 'application_id', v_existing_id, 'found_by', 'tckn');
    END IF;
    
    -- Fallback: Check by member_id (for legacy records)
    SELECT id, tckn INTO v_existing_id, v_existing_tckn
    FROM applications 
    WHERE campaign_id = p_campaign_id AND member_id = p_member_id
    LIMIT 1;
    
    IF v_existing_id IS NOT NULL THEN
        RETURN jsonb_build_object('exists', true, 'application_id', v_existing_id, 'found_by', 'member_id');
    END IF;
...
-- 4. Webhook Trigger Function (for Email)
...
-- 5. Update Secure RPC (Uses Plain TCKN now)
